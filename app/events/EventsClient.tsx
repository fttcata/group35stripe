...
export default function EventsClient({ initialEvents }: { initialEvents: any[] }) {
  ...
  useEffect(() => {
    const submittedEvents = JSON.parse(localStorage.getItem('submittedEvents') || '[]') as any[]
    if (submittedEvents.length > 0) {
      setAllEvents((prev) => [...prev, ...submittedEvents])
    }
  }, [])
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-700 text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-16 sm:py-24">
          <div className="text-center space-y-4">
            <h1 className="text-5xl sm:text-6xl font-extrabold tracking-tight">Upcoming Events</h1>
            <p className="text-xl sm:text-2xl text-purple-100 max-w-2xl mx-auto">Discover amazing events, workshops, and meetups near you</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1 space-y-12">
            {Object.entries(grouped).map(([month, items], idx) => (
              <div key={month} className="animate-fade-in-up" style={{ animationDelay: `${idx * 100}ms` }}>
                ...
                <div className="flex flex-col gap-6">
                  {items.map((e, cardIdx) => (
                    <div key={e.title} className="animate-fade-in-up w-full" style={{ animationDelay: `${(idx * 100) + (cardIdx * 50)}ms` }}>
                      <EventCard
